...
## each split is about 13 for the response variable in the original
## units, closely followed by 12.
numVarSplit <- 13
## ##################################################
...
## RMSE: 193.863589 +/- 2.355363
c(mean(fullRsq), 1.96*sd(fullRsq))
## Rsq: 0.875769841 +/- 0.003018901
...
##   coord_flip() +
##   geom_col() +
##   labs(x="Eukaryotic order-level taxa", y="Decrease in node impurity")
## ggsave(filename="orig_units_all_data_orders_IncNodePurity_barchart.pdf", height=2.5, width=4.5, units="in")
## ## ##################################################
...
ggplot(importanceT %>% top_n(n, wt=`%IncMSE`),
  ...
  coord_flip() +
  geom_col() +
  labs(x="Eukaryotic order-level taxa", y="Mean % decrease in MSE when excluded")
...
ggsave("infl_euk_order_all_data_scatter.pdf", width=8, height=4, units="in")
## ##################################################
