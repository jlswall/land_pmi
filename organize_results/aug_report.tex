...
While this requirement may seem unusual, it prevents a rare taxon
found on just one unusual individual from adversely affecting the
model.  In my earlier models using bacterial family-level taxa, it had
happened that a particular taxon, Listeriaceae, that made up more than
1\% of the taxa on only one cadaver for only one day had been included
as an important predictor in the final model.  My concern is that
Listeriaceae may not be present on future observed cadavers, or,
because it is at such low levels, it may not always be easily
measured.  To obtain a more usable model, I wanted to avoid including
taxa which appeared very infrequently in our samples, or were
introduced purely through one unusual cadaver.
...
cadaver-day combinations mentioned in the data description section).


\subsubsection{Results for family-level taxa}

Figure \ref{fig:infl_bac_family_taxa} shows the same type of
...
\begin{figure}
  \centering
  \includegraphics{../revise_algorithm/only_families/all_time_steps/hit_1perc_twice/orig_units_all_data_families_barchart}
...
that were left out.  Figure \ref{fig:resids_cv_bac_family_taxa}
...
Figure \ref{fig:leave_one_out_resids_bac_family_taxa} shows the residuals
...
\begin{figure}
  ...
  \caption{Residuals for model runs leaving out one subject and one day (family-level bacterial taxa)}
