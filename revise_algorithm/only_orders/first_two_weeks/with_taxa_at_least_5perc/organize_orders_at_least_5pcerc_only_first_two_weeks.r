...
## "Rare" taxa group.  Now, I'm making the cutoff at 5%, AND I'm only
## using data for the first couple of weeks, so I need to check that
## all of these taxa have at least one day during the first couple of
## weeks where the frequency exceeds the NEW cutoff for at least one
## individual.  Otherwise, those taxa should be considered "rare" for
## an analysis of just the first 2 weeks.
freqCutoff <- 0.05
...
write.csv(commontaxaT, file="orders_at_least_5perc_only_first_two_weeks.csv", row.names=FALSE)
## ##################################################
